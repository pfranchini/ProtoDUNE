...
private:
  ...
  int number_of_fragments = 24; // 4 SSPs
...
void dune::SSPRawDecoder::reconfigure(fhicl::ParameterSet const& pset) {
 ...
 NOvAClockFrequency=pset.get<double>("NOvAClockFrequency"); // in MHz
  ...
  peaks_min=pset.get<int>("SSP_PEAKS_min");
  peaks_max=pset.get<int>("SSP_PEAKS_max");
  // TODO
  //  - Add a couple fcls to hold the max and min values for some of the histograms
  ...
  std::cout << "Number of fragments: " << number_of_fragments << std::endl;std::cout << "Fragment: " << fragment << std::endl;
...
void dune::SSPRawDecoder::setRootObjects(){
  ...
  peaks_all_ = tFileService->make<TH1D>("peaks","Peaks height distribution",100,peaks_min,peaks_max);
  ...
  for (int i=0;i<number_of_packets*number_of_fragments;i++) {
    ...
    peaks_[i] = tFileService->make<TH1D>(Form("peaks_channel_%d",i),Form("peaks_channel_%d",i), 100,peaks_min,peaks_max);
