...
{
  art::ServiceHandle<art::TFileServiceOM> fs;
...
void dune::SSPRawDecoder::setRootObjects(){
  art::ServiceHandle<art::TFileServiceOM> tFileService;
...
void dune::SSPRawDecoder::readHeader(const SSPDAQ::EventHeader* daqHeader, struct trig_variables* tv){
  ...
  tv->header = daqHeader->header;
  ///> get the length of the packet in unsigned ints (including header)
  tv->length = daqHeader->length;
  ///> get the packet type
  tv->type = ((daqHeader->group1 & 0xFF00) >> 8);
  ///> get the status flags
  tv->status_flags = ((daqHeader->group1 & 0x00F0) >> 4);
  ///> get the header type
  tv->header_type = ((daqHeader->group1 & 0x000F) >> 0);
  ///> get the packet ID
  tv->trig_id = daqHeader->triggerID;
  ///> get the module ID
  tv->module_id = ((daqHeader->group2 & 0xFFF0) >> 4);
  ///> get the channel ID
  tv->channel_id = ((daqHeader->group2 & 0x000F) >> 0);
  ...
  ///> get the peak sum
  tv->peaksum = ((daqHeader->group3 & 0x00FF) >> 16) + daqHeader->peakSumLow;
  if(tv->peaksum & 0x00800000) {
    tv->peaksum |= 0xFF000000;
  }
  ///> get the peak time
  tv->peaktime = ((daqHeader->group3 & 0xFF00) >> 8);
  ///> get the prerise
  tv->prerise = ((daqHeader->group4 & 0x00FF) << 16) + daqHeader->preriseLow;
  ///> get the integrated sum
  tv->intsum = ((unsigned int)(daqHeader->intSumHigh) << 8) + (((unsigned int)(daqHeader->group4) & 0xFF00) >> 8);
  ///> get the baseline
  tv->baseline = daqHeader->baseline;
  ...
  tv->internal_interpol = daqHeader->intTimestamp[0];
...
void dune::SSPRawDecoder::produce(art::Event & evt){
  ...
  // DAQToOffline::SSPReformatterAlgs sspReform(pset2); !! //.get<fhicl::ParameterSet>("SSPReformatter"));
  // sspReform.DAQToOffline::SSPReformatterAlgs::SSPFragmentToWaveformsAndHits(fragments, opDetWaveformVector, opHitVector);     
  ...
  for(auto const& frag: fragments){
	...
	if(verb_values) {
      ...
      // fill ophit
      //      hits.emplace_back( recob::OpHit DAQToOffline::SSPReformatterAlgs::ConstructOpHit(daqHeader) );
